module ECPM (
    ...
    typedef enum logic [3:0] {IDLE, INIT,INIT_2, COMPUTE, WAIT_ECPA, WAIT_ECPD, PRE_COMPUTE, PRE_DONE, DONE} state_t;
    state_t state;
    ...
    if (!i_rst_n) begin
        ...
        case (state)
            ...
            INIT: begin
                if (ecpd_done) begin
                    ecpd_start <= 0;
                    X1 <= X_ecpd; Y1 <= Y_ecpd; Z1 <= Z_ecpd; // R1 = 2P
                    X0 <= X; Y0 <= Y; Z0 <= Z; // R0 = P
